...
class AdvancedTools {
    ...
    async performSensitivityAnalysis(params) {
        try {
            // המרה בטוחה של הערכים מהטופס
            const baseReturn = parseFloat(params.annualReturn) || 6.0;
            const baseFee = parseFloat(params.annualFee) || 0.85;
            ...
            // כל תרחיש ירוץ עם הפרמטרים המקוריים שלו ללא הגבלות זמן
            console.log('Starting sensitivity analysis with original params:', params);
            ...
            for (const scenario of scenarios) {
                ...
                // כל תרחיש יעבוד עם הגדרות המשתמש המקוריות ללא הגבלות
                // אם המשתמש ביקש מקסימום חודשים - כל תרחיש יקבל מקסימום לפי התנאים שלו
                // אם המשתמש ביקש מספר חודשים מסוים - כל תרחיש יקבל את אותו מספר
                
                console.log(`Calculating scenario "${scenario.name}" with params:`, {
                    returnRate: newReturn,
                    feeRate: newFee,
                    showMaxMonths: testParams.showMaxMonths,
                    requestedMonths: testParams.requestedMonths
                });
            ...
            return {
                ...
                subtitle: 'כל תרחיש מדמה סימולציה עצמאית עם הפרמטרים המעודכנים',
                scenarios: results,
                baseScenario: baseResult,
                isIndependentSimulation: true, // מציין שכל תרחיש הוא סימולציה עצמאית
