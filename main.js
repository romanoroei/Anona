...
createCapitalCompositionChart(results) {
        ...
        try {
            ...
            if (panel) {
                ...
                let html = '<ul class="numbers-list">';
...
// טיפול בשדה האינפלציה
    ...
    displaySummaryCards(results) {
        // עדכון כותרת המשיכה החודשית - הפוך מהשיטה שבחר המשתמש
        ...
        const elements = {
            ...
            summaryWithdrawal: results.summary.averageWithdrawalByMethod,
    ...
    hideTable() {
        const tableContainer = document.getElementById('tableContainer');
        const tableCloseSection = document.getElementById('tableCloseSection');
        if (tableContainer) {
            tableContainer.style.display = 'none';
        }
        if (tableCloseSection) {
            tableCloseSection.style.display = 'none';
        }
    ...
    displayTable(results, monthsLimit = null) {
        ...
        const tableCloseSection = document.getElementById('tableCloseSection');
        if (tableCloseSection) {
            tableCloseSection.style.display = 'block';
        }
    ...
    formatSensitivityResults(results) {
        ...
        if (results.scenarios && results.scenarios.length > 0) {
            ...
            results.scenarios.forEach(s => {
                ...
                const scenarioClass = s.scenario === 'תרחיש בסיסי' ? ' class="base-scenario"' : '';
                html += `<tr${scenarioClass}>
                    <td><strong>${s.scenario === 'תרחיש בסיסי' ? '🎯 ' : ''}${s.scenario}</strong></td>
